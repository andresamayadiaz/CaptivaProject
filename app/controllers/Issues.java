...
@With(Security.class)
@Check("any")
public class Issues extends BaseController {
    ...
    public static void index() {
		...
		if(params.get("statusFilter") != null) {			
			if(params.get("statusFilter").equals("true")){ // open
				List<Issue> entities = Issue.find("(?1 is null or Owner = ?1) and isOpen = true ORDER BY DueDate DESC", user != null ? user : Security.getConnectedUser()).fetch();
				...
				List<Issue> entities = Issue.find("(?1 is null or Owner = ?1) and isOpen = false ORDER BY DueDate DESC", user != null ? user : Security.getConnectedUser()).fetch();
				renderArgs.put("statusFilter", "false");
				renderArgs.put("userFilter", user != null ? user.id : "none");
		        render(entities);
			} else { // all
				List<Issue> entities = Issue.find("(?1 is null or Owner = ?1) ORDER BY DueDate DESC", user != null ? user : Security.getConnectedUser()).fetch();
    ...
    public static void create(Issue entity) {
        render(entity);
    }
    ...
    public static void save(@Valid Issue entity) {
        if (validation.hasErrors()) {
            flash.error(Messages.get("scaffold.validation"));
            render("@create", entity);
        }
        //Logger.info("Saving Issue: %s", "Name: "+entity.Name+" / Desc: "+entity.Description+" / Owner: "+entity.Owner.id +" / Milestone: "+entity.Milestone.id);
        entity.save();
        flash.success(Messages.get("scaffold.created", "Task"));
        show(entity.id);
    }
    
    public static void update(@Valid Issue entity) {
    ...
    public static void getGraph(java.lang.Long id) {
    	...
    	// Days
    	Time lastTime = Time.find("Issue = ? ORDER BY created DESC", entity).first();
    	...
    	// json
    	List<List<Double>> issueGraph = new ArrayList<List<Double>>(); 
    	...
    	issueGraph.add(actualTimeList);
    	issueGraph.add(estimatedTimeList);
    	
    	//Logger.info("JSON issueGraph: %s", new Gson().toJson(issueGraph));
    	
    	renderJSON(issueGraph);
    }
